...
두 파일 모두 `pam_fprintd.so max-tries=10 timeout=30` 을 쓴다.
지문 인증은 최대 10회 시도하고 실패하면 기존 비밀번호 인증으로 넘어간다.
시간 초과나 센서 오류는 더 일찍 폴백할 수 있다. 연속 영상의 프레임 수가
아니라 PAM 지문 인증 시도 횟수이며, 반복 시도가 성공을 보장하지는 않는다.
30초 제한은 손가락이 감지되지 않을 때 55x4 센서를 오래 활성화해 과열
보호가 동작하는 일을 막는다. 이미 설치된 설정은 `--repair-pam`으로
안전하게 보정할 수 있다. 기존 설정을 보존하는 적용 도구와 관리자 권한 실행
방법은 [관리자 인증 안내](14-admin-auth-notice.md)를 참고한다.
...
4. 10번 다 실패하거나 시간 제한에 도달하면 -> 비밀번호 프롬프트로 넘어간다
```

경과 시간이나 "10번 중 3번째" 같은 카운터는 없다. `pam_fprintd` 는 이
