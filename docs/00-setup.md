...
두 파일 모두 `pam_fprintd.so max-tries=10 timeout=90` 을 쓴다.
...
이미 설치된 20회 설정은 자동으로 바뀌지 않는다. 기존 설정을 보존하는
적용 도구와 관리자 권한 실행 방법은 [관리자 인증 안내](14-admin-auth-notice.md)를 참고한다.
