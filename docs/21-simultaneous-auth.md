...
취소 정리에는 VerifyStop과 Release의 제한 시간을 모두 수용하는 1.5초 유예를
두어 worker가 장치 해제 전에 강제 종료되어 fprintd claim이 고착되지 않게 한다.
...
Plasma 6.7 잠금 화면은 비대화형 인증기의 일반 PAM 정보 신호를 기본 테마의
지문 라벨에 표시하지 않고 오류 신호만 연결한다. `fingerprint-only` 모드는 이
표시 채널로 준비, 접촉, 확인 애니메이션, 불일치 횟수 및 종료 상태를 보낸다.
이는 화면 표시 방식만 바꾸며 인증 성공 여부에는 관여하지 않는다. 센서 응답이
없거나 60초/20회 제한에 도달하면 `비밀번호를 직접 입력하세요`라고 명시한다.
sudo·TTY·Polkit의 기존 정보 메시지 스타일은 그대로 유지한다.
...
`LC_ALL` → `LC_MESSAGES` → `LANGUAGE` → `LANG` 순서이며 미지원 언어는 영어로 표시한다.
gettext 형식의 `LANGUAGE=ko:en` 우선순위 목록도 지원한다.
GUI의 수동 언어 선택과 시스템 인증 언어는 별도 설정이다. GUI와 위저드의
문구는 모두 Qt 번역 카탈로그를 거치며, 새 고급 문구의 번역이 없는 언어는
영어 원문으로 안전하게 대체된다.
