...
int main(void)
{
    ...
    unsetenv("LC_MESSAGES"); setenv("LANGUAGE", "ko:en", 1);
    if (strcmp(fp_message_catalog()->language, "ko")) return 9;
    setenv("LC_MESSAGES", "fr_FR.UTF-8", 1);
    if (strcmp(fp_message_catalog()->language, "fr")) return 10;
    ...
    puts("11 PAM languages, LANGUAGE lists, locale precedence and fallback passed");
    return 0;
}
